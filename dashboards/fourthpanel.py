...
def get_total_count(keyword, api_key=None):
...
if st.sidebar.button("Search PubMed"):
    if keyword:
        with st.spinner("Searching PubMed..."):
            try:
                total_count = get_total_count(keyword, api_key=os.environ.get("NCBI_API_KEY"))
                ...
                for i, article in enumerate(articles, 1):
                    ...
                    st.markdown(f"**First Author:** [{article['First Author']}](https://pubmed.ncbi.nlm.nih.gov/?term={article['First Author']})")
                    st.markdown(f"**Title:** [{article['Title']}](https://pubmed.ncbi.nlm.nih.gov/{article['PubMed ID']}/)")
...
api_key = st.sidebar.text_input("Enter NCBI API Key (if required)", "")
if api_key:
